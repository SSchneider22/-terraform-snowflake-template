...
resource "snowflake_grant_account_role" "access_role_to_functional_role_grants" {
  for_each = { for item in local.grant_access_role_to_functional_role : "${item.access_role}-${item.functional_role}" => item }
...
resource "snowflake_grant_database_role" "access_role_to_functional_role_grants" {
  for_each = { for item in local.grant_access_db_role_to_functional_role : "${item.access_role}-${item.functional_role}" => item }
